...
export default function GlobeVisualization(): JSX.Element {
  ...
  useEffect(() => {
    ...
    const drawGlobe = (timestamp: number) => {
      ...
      if (!interactionRef.current.isMouseDown) {
        interactionRef.current.rotationY +=
          interactionRef.current.rotationSpeedY;
        interactionRef.current.rotationX +=
          interactionRef.current.rotationSpeedX;
      }
      ...
      particles.forEach((particle, i) => {
        ...
        const offsetLng =
          particle.lng +
          Math.sin(
            animationProps.particleTime * particle.speed + particle.offset
          ) *
            5;
      ...
      connections.forEach((connection, idx) => {
        ...
        if (pointA && pointB && pointA.z < 0 && pointB.z < 0) {
          ...
          const pulseX =
            pointA.projX + (pointB.projX - pointA.projX) * pulsePos;
          const pulseY =
            pointA.projY + (pointB.projY - pointA.projY) * pulsePos;
      ...
      projectedPoints.forEach((point) => {
        if (point.z < 0) {
          // Only draw points on the front hemisphere
          const isHovered =
            animationProps.hoverPoint &&
            animationProps.hoverPoint.index === point.index;
          ...
          drawGlowingCircle(
            ...
            isHovered
              ...
              : `rgba(79, 195, 247, ${baseOpacity * 0.5})`,
            pointSize * 2
          );
        }
      });
      ...
      if (animationProps.hoverPoint) {
        ...
        ctx.fillText(
          point.name,
          animationProps.hoverPoint.x,
          animationProps.hoverPoint.y - 20
        );
      }
    };
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const radius =
        Math.min(rect.width, rect.height) * 0.35 * interactionRef.current.zoom;
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      interactionRef.current.zoom = Math.max(
        0.5,
        Math.min(2, interactionRef.current.zoom + zoomDelta)
      );
    };
  ...
  return (
    <canvas
      ref={canvasRef}
      className={`globe-canvas w-full h-full block ${
        isInteracting ? "shadow-[0_0_30px_rgba(59,130,246,0.5)]" : ""
      }`}
    />
  );
}
